...
export const bookings = pgTable(
  'bookings',
  {
    ...
    // Reminder dispatch (24h pre-appointment cron sets this on send)
...
// ── booking_events ──
// Append-only audit trail. One row per material change to a booking; the
// /admin change-log inline panel renders these in reverse chronological order.
export const bookingEvents = pgTable(
  'booking_events',
  {
    ...
    // Event types: created | services_edited | status_changed | rescheduled |
    //   cancelled | balance_link_sent | balance_paid | refund_completed |
    //   refund_failed | reminder_sent | confirmation_sent | update_notified
    eventType: text('event_type').notNull(),
    summary: text('summary').notNull(),
    payload: jsonb('payload'), // structured before/after diffs
    actor: text('actor'), // 'admin:<email>' | 'customer' | 'system' | 'webhook'
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => ({
    bookingCodeIdx: index('booking_events_code_idx').on(table.bookingCode),
    createdAtIdx: index('booking_events_created_at_idx').on(table.createdAt),
  }),
);
