...
/** Admin: fetch the full audit-log event list for a booking. */
export async function fetchBookingEvents(code) {
  const res = await fetch(`/api/bookings/${encodeURIComponent(code)}/events`, {
    cache: 'no-store',
  });
  const data = await handleJson(res);
  return data.events || [];
...
/** Admin: replace the booking's service list. The server recomputes totals
 *  and returns { booking, creditOwedCents, delta } so the UI can hint when
 *  a refund is appropriate. */
export async function updateBookingServices(code, { serviceIds, addOnNames } = {}) {
  const res = await fetch(`/api/bookings/${encodeURIComponent(code)}/services`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ serviceIds, addOnNames }),
  });
...
/** Fetch the taken time-slot ids for a date. Pass the candidate service IDs
 *  (multi-service bookings) so the API sums durations and blocks slots that
 *  would overlap any existing booking. Accepts a single id (legacy) or array. */
export async function fetchTakenSlotsForDate(dateKey, serviceIds) {
  const params = new URLSearchParams({ date: dateKey });
  if (Array.isArray(serviceIds) && serviceIds.length > 0) {
    params.set('serviceIds', serviceIds.join(','));
  } else if (typeof serviceIds === 'string' && serviceIds) {
