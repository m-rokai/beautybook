...
export async function findBookingByPaymentId(paymentId) {
  const rows = await db
    ...
    .where(
      or(
        eq(bookings.depositSquarePaymentId, paymentId),
        eq(bookings.balanceSquarePaymentId, paymentId),
      ),
    )
    .limit(1);
  return rows[0] ?? null;
...
export async function findBookingByRefundId(refundId) {
  const rows = await db
    .select()
    .from(bookings)
    .where(
      or(
        eq(bookings.refundDepositSquareId, refundId),
        eq(bookings.refundBalanceSquareId, refundId),
      ),
    )
    .limit(1);
  return rows[0] ?? null;
